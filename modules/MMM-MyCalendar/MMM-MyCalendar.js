    ...
    for (var e in events) {
      ...
      if (this.config.displaySymbol) {
        ...
        use.setAttributeNS(
          ...
          "href",
          "modules/MMM-MyCalendar/icon_sprite.svg#calendar"
        );
        svg.appendChild(use);
      ...
      if (event.fullDayEvent) {
        if (event.today) {
          ...
          event.startDate - now < 6 * oneDay &&
          event.startDate - now > 0
        ) {
          timeWrapper.innerHTML = this.capFirst(
            moment(event.startDate, "x").format(this.config.dayOfWeekFormat)
          );
        } else {
        ...
        if (event.startDate >= new Date()) {
          if (event.startDate - now < 6 * oneDay) {
            if (event.startDate - now < this.config.getRelative * oneHour) {
              ...
              timeWrapper.innerHTML =
                this.capFirst(this.translate("TODAY")) +
                " " +
                this.config.joiningWord +
                " " +
                this.capFirst(
                  moment(event.startDate, "x").format(this.config.timeFormat)
                );
            } else if (
              momentEventStart.isSame(moment(momentNow).add(1, "day"), "day")
            ) {
              // This event is tomorrow
              timeWrapper.innerHTML =
                this.capFirst(this.translate("TOMORROW")) +
                " " +
                this.config.joiningWord +
                " " +
                this.capFirst(
            ...
            if (!this.config.useRelativeDates) {
              if (
                this.config.urgency > 1 &&
                momentEventStart.isSameOrBefore(
                  moment(momentNow).add(this.config.urgency, "days")
                )
              ) {
                // This event falls within the config.urgency period that the user has set
                timeWrapper.innerHTML = this.capFirst(
          ...
          timeWrapper.innerHTML =
            this.capFirst(this.translate("RUNNING")) +
            " " +
      ...
      if (event.location && this.config.showLocation) {
        ...
        attendeesWrapper.innerHTML = "Penanggung Jawab		: " + event.attendees;
